...
(function() {
  "use strict";
  var BlipTv, Html5Video, IFrameSlide, ImgSlide, NoSlide, Presentz, RvlIO, SambatechPlugin, SlideShare, SlideShareOEmbed, SpeakerDeck, SwfSlide, Video, Vimeo, WistiaPlugin, Youtube, root,
  ...
  SambatechPlugin = (function() {
    ...
    function SambatechPlugin(presentz, videoContainer, width, height) {
      ...
      this.video = new Video(["play"], ["pause"], ["end"], this.presentz);
      this.elementId = this.presentz.newElementName();
      this.videoData = null;
      this.time = 0;
    }

    SambatechPlugin.prototype.ensureSambatechIframeAPILoaded = function(callback) {
    ...
    SambatechPlugin.prototype.changeVideo = function(videoData, wouldPlay) {
      this.wouldPlay = wouldPlay;
      this.videoData = videoData;
      return this.ensureSambatechIframeAPILoaded((function(_this) {
        return function() {
          root.videoSamba = _this.video;
          root.wouldPlaySamba = _this.wouldPlay;
          _this.player = new SambaPlayer("player_video", {
            height: _this.height,
            width: _this.width,
            ph: _this.playerKey(videoData),
            m: _this.videoId(videoData),
            events: {
              onLoad: "onLoadSamba",
    ...
    SambatechPlugin.prototype.videoId = function(videoData) {
      var id, lowercaseUrl;
    ...
    SambatechPlugin.prototype.playerKey = function(videoData) {
      var id, lowercaseUrl;
      lowercaseUrl = videoData.url.toLowerCase();
      id = videoData.url;
      if (lowercaseUrl.indexOf("http://player.sambatech.com.br/current/samba-player.js") !== -1) {
    ...
    SambatechPlugin.prototype.handleEvent = function(event) {
    ...
    SambatechPlugin.prototype.skipTo = function(time, wouldPlay) {
      var _ref;
      if (wouldPlay == null) {
        wouldPlay = false;
      }
      if (((_ref = this.player) != null ? _ref.seek : void 0) != null) {
    ...
    SambatechPlugin.prototype.pause = function() {
      return this.player.pause();
    };

    SambatechPlugin.prototype.isPaused = function() {
      return this.video.isInPauseState;
    };

    SambatechPlugin.prototype.onStartSamba = function() {
      return videoSamba.handleEvent("play");
    };

    SambatechPlugin.prototype.onPauseSamba = function() {
      return videoSamba.handleEvent("pause");
    };

    SambatechPlugin.prototype.onFinishedSamba = function() {
      return videoSamba.handleEvent("end");
    };

    SambatechPlugin.prototype.onLoadSamba = function() {
      if (wouldPlaySamba) {
        return videoSamba.play();
      }
    };

    return SambatechPlugin;
  ...
  WistiaPlugin = (function() {
    var IFRAME_API;

    IFRAME_API = "//fast.wistia.com/assets/external/E-v1.js";

    function WistiaPlugin(presentz, videoContainer, width, height) {
      this.presentz = presentz;
      this.videoContainer = videoContainer;
      this.width = width;
      this.height = height;
    ...
    WistiaPlugin.prototype.ensureWistiaIframeAPILoaded = function(callback) {
      var $scripts, firstScriptTag, script;
      if (jQuery("script[src=\"" + IFRAME_API + "\"]").length === 0) {
        script = document.createElement("script");
        script.type = "text/javascript";
    ...
    WistiaPlugin.prototype.changeVideo = function(videoData, wouldPlay) {
      this.wouldPlay = wouldPlay;
      jQuery(this.videoContainer).append("<div id='wistia_" + (this.videoId(videoData)) + "' class='wistia_embed' style='width:" + this.width + "px;height:" + this.height + "px;'>&nbsp;</div>");
      return this.ensureWistiaIframeAPILoaded((function(_this) {
        return function() {
          var video;
          jQuery(_this.videoContainer).append("<div id=\"" + _this.elementId + "\"></div>");
          _this.player = Wistia.embed(_this.videoId(videoData), {
            version: "v1",
            videoWidth: _this.width,
            videoHeight: _this.height,
            playerColor: "688AAD"
          });
          _this.player.bind("ready", function() {
            return this.onReady;
          });
          video = _this.video;
          _this.player.bind("play", function() {
            return video.handleEvent("play");
          });
          _this.player.bind("pause", function() {
            return video.handleEvent("pause");
          });
          _this.player.bind("end", function() {
            return video.handleEvent("end");
          });
        };
      })(this));
    };

    WistiaPlugin.prototype.videoId = function(videoData) {
      var id, lowercaseUrl;
      lowercaseUrl = videoData.url.toLowerCase();
      id = videoData.url;
      if (lowercaseUrl.indexOf("//www.wistia.com/") !== -1) {
    ...
    WistiaPlugin.prototype.onReady = function() {
      if (this.wouldPlay) {
        return this.play();
      }
    };

    WistiaPlugin.prototype.handleEvent = function(event) {
      this.video.handleEvent(event.data);
    };

    WistiaPlugin.prototype.handle = function(video) {
      var lowerCaseUrl;
      lowerCaseUrl = video.url.toLowerCase();
      return lowerCaseUrl.indexOf("//www.wistia.com/") !== -1;
    };

    WistiaPlugin.prototype.currentTime = function() {
      var _ref;
      if (((_ref = this.player) != null ? _ref.time : void 0) != null) {
        return this.player.time();
      }
      return 0;
    };

    WistiaPlugin.prototype.skipTo = function(time, wouldPlay) {
      var _ref;
      if (wouldPlay == null) {
        wouldPlay = false;
      }
    ...
    WistiaPlugin.prototype.play = function() {
      return this.player.playVideo();
    };

    WistiaPlugin.prototype.pause = function() {
      return this.player.pauseVideo();
    };

    WistiaPlugin.prototype.isPaused = function() {
      return this.video.isInPauseState;
    };
  ...
  Vimeo = (function() {
    ...
    Vimeo.prototype.onReady = function(id) {
      this.player = $f(id);
      this.player.addEvent("play", (function(_this) {
        return function() {
          _this.video.handleEvent("play");
        };
      })(this));
      this.player.addEvent("pause", (function(_this) {
        return function() {
          _this.video.handleEvent("pause");
        };
      })(this));
      this.player.addEvent("finish", (function(_this) {
        return function() {
          _this.video.handleEvent("finish");
        };
      })(this));
      this.player.addEvent("playProgress", (function(_this) {
        return function(data) {
          _this.currentTimeInSeconds = data.seconds;
        };
      })(this));
      this.player.addEvent("loadProgress", (function(_this) {
        return function(data) {
  ...
  Youtube = (function() {
    ...
    Youtube.prototype.changeVideo = function(videoData, wouldPlay) {
      this.wouldPlay = wouldPlay;
      this.ensureYoutubeIframeAPILoaded((function(_this) {
        return function() {
  ...
  SlideShareOEmbed = (function() {
    ...
    SlideShareOEmbed.prototype.changeSlide = function(slide) {
      ...
      this.ensureSlideInfoFetched(slide.public_url, (function(_this) {
        return function() {
          var $img;
  ...
  SwfSlide = (function() {
    ...
    SwfSlide.prototype.preload = function(slide) {
      ...
      swfobject.embedSWF(slide.url, "" + this.preloadSlideContainerId, "1", "1", "8", null, null, null, atts, (function(_this) {
        return function() {
  ...
  Presentz = (function() {
    ...
    function Presentz(videoContainer, videoWxH, slideContainer, slideWxH) {
      ...
      this.availableVideoPlugins = {
        ...
        wistia_plugin: new WistiaPlugin(this, videoContainer, sizeOfVideo.width, sizeOfVideo.height),
        sambatech_plugin: new SambatechPlugin(this, videoContainer, sizeOfVideo.width, sizeOfVideo.height),
    ...
    Presentz.prototype.startTimeChecker = function() {
      ...
      timeChecker = (function(_this) {
        return function() {
          var clazz, plugin;
          plugin = _this.presentation.chapters[_this.currentChapterIndex].video._plugin;
          if (plugin instanceof SambatechPlugin) {
            clazz = _this;
            plugin.player.getStatus(function(p) {
              if (clazz.currentChapterIndex !== -1) {
                return clazz.checkSlideChange(p.status.time);
              }
            });
          } else {
